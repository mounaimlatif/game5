<?php
/**
 * @version		$Id: route.php 1828 2013-01-25 13:40:54Z lefteris.kavadas $
 * @package		K2
		...
		foreach ($needles as $needle => $id)
		{
			if (count($items))
			{
				...
				if (is_null($match))
				{
					foreach ($items as $item)
					{
						if ($needle == 'category')
						{
							...
							if (isset($item->K2Categories) && is_array($item->K2Categories))
							{
								...
								if (count($item->K2Categories) === 0 && @$item->query['view'] == 'itemlist' && @$item->query['task'] == '')
								{
									$anyCategoryLink = $item;
								}
							}
						}
						if (!is_null($match))
					...
					if (is_null($match) && $needle == 'category' && isset($anyCategoryLink))
					{
						$match = $anyCategoryLink;
					}
