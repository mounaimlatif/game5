...
(function($){
    ...
    function YoxView()
    {
        ...
        var defaults = {
            ...
            cacheBuffer:0, // The number of images to cache after the current image (directional, depends on the current viewing direction)
        ...
        function getElementDimensions(type, originalDimensions, options)
        {
			...
			var size = options.defaultDimensions[type];
            if (isNaN(size.width))
                size.width = null;
        ...
        var supportedTypes = {
            ...
            flash: function(thumbnail, thumbnailHref, thumbImg, options)
            {
                ...
                if (matchFlash || matchFlashVideo)
                {
                    ...
                    flashPanel.addClass('yoxview_element');
            ...
            iframe: function(thumbnail, thumbnailHref, thumbImg, options)
            {
                ...
                if (thumbnailTarget && thumbnailTarget == "yoxview")
                {
                    ...
                    if (urlData && urlData.path)
                    {
                        ...
                        if(Yox.urlDataToPath(urlData).indexOf('?') == -1){
							iframeurl = Yox.urlDataToPath(urlData)+'?tmpl=component&width='+options.defaultDimensions['iframe'].width+'&height='+options.defaultDimensions['iframe'].height;
						} else {
							iframeurl =Yox.urlDataToPath(urlData)+'&tmpl=component&width='+options.defaultDimensions['iframe'].width+'&height='+options.defaultDimensions['iframe'].height;
						}
                        imageData = {
                            "element": $("<iframe>", {
                                src: iframeurl
                            }).addClass('yoxview_element').attr("id","bd-popup-iframe"),
    ...
    //    callback: A function to call after the gallery has opened.
        ...
        {
            var isJquery = this instanceof $jacp;
            if (isJquery)
            {
            ...
            popupWrap.stop().css({ opacity: 1, display: "block" });
            if(options.cacheImagesInBackground)
    ...
    //                on the keyboard, specify the Next button. If no button should be display, leave blank.
        ...
        this.close = function()
        {
    		...
    		popupWrap.stop().css({"opacity":0,"display":"none"});
		    if (isPlaying)
			    stopPlay();
        ...
        {      
            ...
            navBtn.addClass('yoxview_ctlBtn');
            ...
            if (options.renderInfo !== false)
            {
			    ...
			    {  
				    ...
				    for (infoButton in options.infoButtons)
				    {
					    ...
					    if(singleImage){
					    	options.infoButtons[infoButton].css("display", "none");
					    }
        ...
        {
            //popup.stop().css(popupPosition, options.popupResizeTime, callback);        
            windowWidth = $(window).width(); //retrieve current window width
			windowHeight = $(window).height(); //retrieve current window height
			
			if(windowWidth < options.maxWidth || windowHeight < options.maxHeight){
				popupPosition.width 	= windowWidth;
            	popupPosition.height = windowHeight;
            	popupPosition.top	= 0;
            	popupPosition.left 	= 0;
        	}
        	
            popup.stop().css(popupPosition).fadeTo('fast',1, callback);
        ...
        function getImagePosition(imageSize)
        {
            ...
            if(imagePosition.height > $(window).height()){
            	imagePosition.height = $(window).height();
            }
        
            imagePosition.top = Math.round(($(window).height() - imagePosition.height) / 2);
            //alert($(window).height());
            //imagePosition.top = options.toppopup;
        ...
        this.resize = function(updateInfoPanel)
        {
            ...
            if(newImagePosition.height > $(window).height()){
            	newImagePosition.height =  $(window).height();           
        	}
            //newImagePosition.height += $(".yoxview_popupBarPanel").height() + $("#yoxview_infoPanel").height();
            ...
            resizePopup(newImagePosition,
                function(){
                    //popup.height() -= $(".yoxview_popupBarPanel").height() + $("#yoxview_infoPanel").height(); 
                	var newImageSize = { width: popup.width(), height: popup.height() - $(".yoxview_popupBarPanel").height() - $("#yoxview_infoPanel").height()};
	    ...
	    function changeMedia(media)
	    {
		    ...
		    else
            {
                ...
                if (!panelData.element)
                {
                    //if (media.html)
                    //{
                        if($('#yoxview').find('.'+mediaPanelClass).length<1){
                        	elementcontent = $("<div>", {
		                    }).addClass(mediaPanelClass).attr("id",mediaPanelClass);
		                    //popup.append(elementcontent);
		                    $('#yoxview_infoPanel').before(elementcontent);
                        }
                    ...
                    //{
                    	...
                    	media.element.load(function() {
                    		  srcelement = $(this).attr("src");
                    		  $(this).contents().find("body").addClass("bd-popup").attr("id","bd-popup-iscroll");
							  $(this).contents().find("body").find("a").each(function(){
									hrefremove = $(this).attr("href");
									if(hrefremove+"?tmpl=component" == srcelement){
                    	...
                    	media.element.bind('load',ifmOnload);
                        elementcontent.append(media.element);
                        //Add class for body content iframe
			...
			windowHeight = $(window).height(); //retrieve current window height
			if(windowWidth < options.maxWidth || windowHeight < options.maxHeight){
				newImageDimensions.width 	= windowWidth;
            	newImageDimensions.height = windowHeight;
            	newImagePosition.top	= 0;
            	newImagePosition.left 	= 0;
        	}
        ...
        function ifmOnload(){
			...
			window.popupIscroll = new iScroll("yoxview_mediaPanel", {vScrollbar: true, hScrollbar: false, scrollbarClass: 'popupTracker', useTransform: false, scroller: (isTouch ? doc.getElementById('ja-mainbody') : null) });
			...
			if($.browser.opera || $.browser.mozilla || ($.browser.msie && $.browser.version >= 9)){
				...
				script.src = yoxviewPath+'scrollevent.js';
				doc.body.appendChild(script);
			}
			*/
			//Add class bd-popup
			$(doc.body).addClass("bd-popup");
			srcelement = $(this).attr("src");
			
			$(doc.body).find('a').each(function(){
				hrefremove = $(this).attr("href");
				
				if(hrefremove.indexOf('?') == -1){
					iframe_url = hrefremove+'?tmpl=component&width='+options.defaultDimensions['iframe'].width+'&height='+options.defaultDimensions['iframe'].height;
				} else {
					iframe_url = hrefremove+'&tmpl=component&width='+options.defaultDimensions['iframe'].width+'&height='+options.defaultDimensions['iframe'].height;
				}
				if(iframe_url == srcelement){
					$(this).replaceWith($(this).html());
				}
        ...
        this.unload = function(callback){
            ...
            function doUnload(){
                ...
                {
                    $jacp.each(loadedViews, function(i, view){
                        removeFromView(view);
                    });
